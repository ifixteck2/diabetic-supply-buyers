...
function findPriceProductForItem(item, priceRows) {
  ...
  if (itemSkus.size) {
    const skuMatch = priceRows.find((product) => hasSharedSku(itemSkus, skuCandidatesFromText(product.product)));
    if (skuMatch) return skuMatch;
  }
  const itemAlias = productAliasKey(`${item.brand || ""} ${item.model || ""}`);
  return priceRows.find((product) => productAliasKey(product.product) === itemAlias)
    || priceRows.find((product) => normalizeMatchText(product.product) === itemText)
